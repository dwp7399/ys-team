...
### `.ys_team/delivery-flow.md` — 核心交付清单

这不是 spec 制品，而是项目本地 baseline 的交付承载位。

职责：

- 固定核心交付主链的阶段交接逻辑
- 在自身作用范围内保证一致性，而不是追求覆盖全部项目细节
- 为项目提供一处显式定义执行起点、状态迁移、证据时机和 close / release 关系的位置

最小结构：

- `Positioning`
  - 说明作用范围、目标和与 `TEAM.md` / `policy.md` / `status.md` / spec 制品的关系
- `Core Flow`
  - 对 `spec-talk`、`spec-review`、`spec-work`、`qa`、`close` 五步分别至少定义：
    - `Goal`
    - `Enter When`
    - `Must Align`
    - `Done When`
...
- `delivery-flow.md` 不替代 `TEAM.md` 的配置职责
- `delivery-flow.md` 不替代 `policy.md` 的治理职责
- 未被 `delivery-flow.md` 覆盖的项目细节，可由其他本地规则补充，但不得与其冲突
