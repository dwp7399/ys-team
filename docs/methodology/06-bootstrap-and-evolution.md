...
| 方法配置 | 弱引导原则、讨论透镜 |
| 治理策略 | 执行规则、文档权威、同步规则 |
| 交付清单 | 项目本地 `delivery-flow.md` 默认骨架 |
...
初始化或重估本地 baseline 时，应同时处理 `.ys_team/delivery-flow.md`：

1. 从 bundled baseline 复制默认骨架
2. 保持其“只覆盖核心交付主链、不追求全覆盖”的定位
3. 要求项目显式补齐会影响主链推进的一组绑定点，而不是把所有细节都塞进文件
4. rebuild 时保留项目已有的本地化决策，只合并 baseline 新增的强制段落
