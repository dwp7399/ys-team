...
装好之后正常和 AI 对话。你不需要判断应该启动哪种工作流，也不需要把需求拆成固定阶段。ys-team 会先看项目现实，再自动判断每个请求该走什么路径：
...
内部流程会吸收规格、测试、审阅、发布前验证等工程纪律，但这些是 ys-team 的路由责任，不是用户的选择负担。你只需要继续描述目标、约束和反馈。

如果输出里的技术名词太多，可以在 `.ys_team/config.yaml` 里设置：

```yaml
output_mode: friendly
```

默认是 `output_mode: technical`，只保留技术输出和必要状态标志。`friendly` 会保留原始技术输出，再追加一段给非程序背景用户看的友好总结。它不强制结构，可以是一句话、短段落或少量要点；重点是把“做了什么技术动作”解释成“这对项目意味着什么、是否需要你处理”。它不改变 ys-team 的路由、验证和证据要求。

如果只是本轮想看简单解释，也可以直接说：“这次用人话总结一下”。
...
如果项目已有领域说明、ADR、issue 约定或团队协作规则，rebuild 可以把它们作为现实依据和讨论材料。ys-team 不会默认覆盖这些本地约定；缺失时只提示是否值得补齐。

如果项目已经形成高频、领域强、容易漏交付面的工作模式，可以把它沉淀为项目本地 SOP。常见做法是：在项目本地规则中声明触发条件，在 checklist 的 close 阶段加入条件式 gate，并把稳定 references 放在项目自己的本地 skill 或文档里。rebuild 应保留这些本地定制，不把它们覆盖成默认 baseline。

Python 或 Java 项目不需要一开始就写复杂 SOP。低成本开始方式是：先完成 init，让 ys-team 生成现实索引；第一次遇到复杂功能时用 spec 收敛；等同类任务重复出现、且已经能说清楚每次必须检查的证据，再把它沉淀为项目本地 SOP。
