...
项目也可以把高频、领域强、容易漏交付面的工作沉淀为项目本地 SOP。ys-team 只负责识别并执行这些本地约定，不把项目业务知识写入通用方法论，也不要求用户手动选择新入口。
...
- 澄清理念或边界 → 不强行写 spec
- 请求命中项目本地 SOP 声明的高频领域场景 → 仍先做 L0/L1/L2 路由，再加载本地 SOP 的条件式交付约束
...
3. 识别分歧和风险
4. 收敛讨论，形成结论；若发现同类项目风险反复出现，建议沉淀为项目本地 SOP，而不是写进 ys-team 核心
5. 如需临时角色，暂停请求用户审批
6. 输出结果卡
